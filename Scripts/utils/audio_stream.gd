extends Node2D

#SFX
const button_press = preload("res://assets/sfx/button_press.mp3")
const cut = preload("res://assets/sfx/tree_cut.wav")
const interact = preload("res://assets/sfx/interact.mp3")
const typing = preload("res://assets/sfx/typing.mp3")
const chop = preload("res://assets/sfx/chop.mp3")
const drop = preload("res://assets/sfx/drop.mp3")

#BGM
const bgm = preload("res://assets/sfx/bgm.mp3")
const bgm_chase = preload("res://assets/sfx/chase_bgm.mp3")
const badjao = preload("res://assets/sfx/badjao.mp3")
const kidnap = preload("res://assets/sfx/kidnap.mp3")
const city = preload("res://assets/sfx/city.mp3")
const piano = preload("res://assets/sfx/piano.mp3")
